...
public class PolyNavAgent : MonoBehaviour{
	...
	void FlipToPos()
    {
        if (activePath[0].x >  this.transform.position.x && this.transform.localScale.x > 0)
        {
			Flip(this.transform);
        }
		else if (activePath[0].x <  this.transform.position.x && this.transform.localScale.x < 0)
        {
			Flip(this.transform);
        }
    }
